...
export default function SingleProject() {
    ...
    return (
        <div className="projectPage m-4">
            {project && <h1 className="title text-center font-bold mb-4 text-secondary" tabIndex={0}>{project.title}</h1>}
                ...
                <div key={index} className='section max-w-full justify-center flex flex-col gap-4 mb-8 sm:flex-row sm:items-center'>
                    {isMobile ? (
                        <>
                            ...
                            <img className='detailsImage' src={project.detailsImage[index]} alt={project.title ? `${project.title} detail image ${index + 1}` : `Project detail image ${index + 1}`} />
                        ...
                        <>
                            {index % 2 === 0 ? (
                                <>
                                    <img className='detailsImage max-w-[50vw] max-h-[60vw] p-2 object-contain' src={project.detailsImage[index]} alt={project.title ? `${project.title} detail image ${index + 1}` : `Project detail image ${index + 1}`} />
